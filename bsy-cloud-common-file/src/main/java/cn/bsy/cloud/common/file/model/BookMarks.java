...
/**
 * @Description: 利用POI进行Word文件相关的操作，针对docx形式的封装
 * @Author gaoh
...
public class BookMarks {
    ...
    private void procParaList(XWPFTableCell cell) {
        ...
        for (XWPFParagraph paragraph : paragraphList) {
            ...
            for (CTBookmark bookmark : bookmarkList) {
                this.bookmarks.put(bookmark.getName(), new BookMark(bookmark, paragraph, cell));
            }
        }
    }
    ...
    private void procParaList(List<XWPFParagraph> paragraphList) {
        for (XWPFParagraph paragraph : paragraphList) {
            ...
            //循环加入标签
            for (CTBookmark bookmark : bookmarkList) {
                this.bookmarks.put(bookmark.getName(), new BookMark(bookmark, paragraph));
            }
        }
    }
                ...
                if (attributes != null) {
                    ...
                    if (colFirstNode != null && colLastNode != null) {
                        ...
                        if (firstColIndex == lastColIndex) {
                            this.bookmarks.put(bookmark.getName(), new BookMark(bookmark, paragraph, tableRow.getCell(firstColIndex)));
                        } else {
